...
from pretix.control.forms.event import (
    CancelSettingsForm, CommentForm, ConfirmTextFormset, EventDeleteForm, FooterLinkSet,
...
class EventUpdate(DecoupleMixin, EventSettingsViewMixin, EventPermissionRequiredMixin, MetaDataEditorMixin, UpdateView):
    ...
    def get_context_data(self, *args, **kwargs) -> dict:
        ...
        context['footer_links'] = self.footer_links
        return context
    ...
    def form_valid(self, form):
        ...
        self.save_footer_links(self.object)
        change_css = False
    ...
    def footer_links(self):
        return FooterLinkSet(self.request.POST if self.request.method == "POST" else None, 
                                        event=self.object,
                                        prefix="footer-links", 
                                        instance=self.object)

    def save_footer_links(self, obj):
        self.footer_links.save()
...
class EventPlugins(EventSettingsViewMixin, EventPermissionRequiredMixin, TemplateView, SingleObjectMixin):
    ...
    def get_context_data(self, *args, **kwargs) -> dict:
        ...
        sorted_plugins = self.sort_plugins(plugins)
        plugins_grouped = self.group_plugins(sorted_plugins)
    ...
    def group_plugins(self, sorted_plugins):
        grouped = groupby(sorted_plugins, lambda p: str(getattr(p, 'category', _('Other'))))
        return [(c, list(plist)) for c, plist in grouped]

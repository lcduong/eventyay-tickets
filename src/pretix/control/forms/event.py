...
class BaseFooterLinkSet(I18nFormSetMixin, forms.BaseInlineFormSet):

    def __init__(self, *args, **kwargs):
        event = kwargs.pop('event', None)
        if event:
            self.set_locales(kwargs, event)
        super().__init__(*args, **kwargs)

    def set_locales(self, kwargs, event):
        kwargs['locales'] = event.settings.get('locales')


FooterLinkSet = inlineformset_factory(
    Event, EventFooterLinkModel,
    EventFooterLinkForm,
    formset=BaseFooterLinkSet,
    can_order=False, can_delete=True, extra=0
)
